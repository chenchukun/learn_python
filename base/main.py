...
from base.package2.sub_package1 import *
def testModule():
    print('-------------testModule-------------')
    ...
    from base.package1.sub_package1 import sub
    sub.sub()
    # 也可以使用import来导入，这种情况下，处理最后一项，其他项必须是一个包，最后一项可以说模块或包，但不能是模块中的名称
    import base.package1.sub_package1.sub
    base.package1.sub_package1.sub.sub()
...
def testFunction():
    print('-------------testFunction-------------')
    # python中函数也一些语句集合，它不需要事先声明，且可以嵌套的定义在其他语句块中
    # 在python中，函数参数可以接收任意类型，在大多数情况下，我们不应该去关系接口对那些特定数据类型有效。
    # python中变量不需要事先声明，在第一次给变量赋值的地方决定了这个变量将存在于哪个命名空间。
    # 变量有三种作用域，def内赋值的变量定位在函数内；在一个嵌套的def中赋值，对于嵌套的函数来说是分本地的；在def外赋值，是整个文件全局的（模块级别的全局）
